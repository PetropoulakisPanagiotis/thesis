                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    for i in range(num_log_images):
                        if args.dataset == 'nyu':
                            ...
                            if canonical:
                                writer.add_image('depth_rc_est0/image/{}'.format(i), colormap_magma(torch.log10(pred_depths_rc_list[0][i, :, :, :].data)), global_step)
                                writer.add_image('depth_rc_est1/image/{}'.format(i), colormap_magma(torch.log10(pred_depths_rc_list[1][i, :, :, :].data)), global_step)
                                writer.add_image('depth_rc_est2/image/{}'.format(i), colormap_magma(torch.log10(pred_depths_rc_list[2][i, :, :, :].data)), global_step)
                                writer.add_image('depth_rc_est3/image/{}'.format(i), colormap_magma(torch.log10(pred_depths_rc_list[3][i, :, :, :].data)), global_step)
                                writer.add_image('depth_rc_est4/image/{}'.format(i), colormap_magma(torch.log10(pred_depths_rc_list[4][i, :, :, :].data)), global_step)
                                writer.add_image('depth_rc_est5/image/{}'.format(i), colormap_magma(torch.log10(pred_depths_rc_list[5][i, :, :, :].data)), global_step)
