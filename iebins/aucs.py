import numpy as np
"""
https://github.com/mattpoggi/mono-uncertainty/blob/543772b4a5165820e94acd3e6ae2349785f763ff/evaluate.py
"""
...
def compute_aucs(gt, pred, uncert, intervals=50):
    ...
    sparse_curve = {
        m: [compute_eigen_errors_v2(gt, pred, metrics=[m], mask=sub, reduce_mean=True)[0] for sub in subs] + [0]
    ...
    opt_curve = {
        m:
        [compute_eigen_errors_v2(gt, pred, metrics=[m], mask=opt_sub, reduce_mean=True)[0]
         for opt_sub in opt_subs[m]] + [0]
        for m in uncertainty_metrics
    }
    ...
    rnd_curve = {
        m:
        [compute_eigen_errors_v2(gt, pred, metrics=[m], mask=None, reduce_mean=True)[0] for t in range(intervals + 1)]
        for m in uncertainty_metrics
    }
...
def SCC(gt, pred, uncertainty_values):
    # Calculate the ranks
    error_values = np.abs(gt - pred)
    error_ranks = np.argsort(error_values)
    uncertainty_ranks = np.argsort(uncertainty_values)
    ...
    spearman_coefficient = 1 - (6 * sum_rank_diff_squared) / (n * (n**2 - 1))
