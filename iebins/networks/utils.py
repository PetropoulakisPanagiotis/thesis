...
def roi_select_features_canonical_shared(feature_map, boxes, labels, downsampling=4, num_semantic_classes=14):
    with torch.no_grad():
        ...
        feature_maps_final = torch.zeros(
            (batch_size, num_semantic_classes, feature_map.shape[1], height, width)).to(labels.device)
        ...
        for class_i in range(num_semantic_classe):
            ...
            if valid_boxes.shape[0] == 0:
                ...
                masks = (zeros_mask + row_mask) * (zeros_mask + col_mask)
                batches_masks = torch.cat([
                    torch.sum(masks[instances_per_batch[i] - 1:instances_per_batch[i + 1]], dim=0).unsqueeze(0)
                ...
                feature_maps_final[instances_per_batch_non_zero_idx[:],
                                   class_i, :, :, :] = masked_feature_map.unsqueeze(1)
...
def pick_predictions_instances_canonical_shared_class(prediction, labels):
    ...
    prediction = prediction[
        torch.tensor(range(0, batch_size * num_semantic_classes *
                           num_semantic_classes, num_semantic_classes)).to(labels.device), :, :]
