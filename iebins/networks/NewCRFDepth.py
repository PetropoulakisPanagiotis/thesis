...
class NewCRFDepth(nn.Module):
    ...
    def forward(self, imgs, epoch=1, step=100, masks=None, instances=None, boxes=None, labels=None):
        ...
        if self.predict_unc == False and self.update_block != 3:
            if (self.update_block >= 9 and self.update_block < 18) or self.update_block == 20 or self.update_block == 21:
                ...
                if self.update_block == 20 or self.update_block == 21:
        ...
        if self.up_mode == 'mask':
            for i in range(max_tree_depth):
                if self.update_block != 20 and self.update_block != 21:
                ...
                if self.update_block != 7 and self.update_block != 8 and self.update_block != 10 and self.update_block != 11 and self.update_block != 12 and self.update_block != 13 and self.update_block != 14 and self.update_block != 15 and self.update_block != 16 and self.update_block != 17 and self.update_block != 20 and self.update_block != 21:
