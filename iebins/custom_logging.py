...
def tb_visualization(writer, global_step, args, current_loss_depth, current_lr, current_loss_unc_decoder, var_sum, var_cnt, num_images, \
    ...
    if args.instances:
        ...
        for i in range(num_images):
            ...
            writer.add_image('image/image/{}'.format(i), inv_normalize(image[i, :, :, :]).data, global_step)
            writer.add_image('depth_gt/image/{}'.format(i), colormap(torch.log10(depth_gt[i, :, :, :].clamp(min=1e-3).data), name='magma'), global_step)
        ...
        for i in range(num_images):
            ...
            # Canonical 
            if args.update_block == 8 or args.update_block == 18:
                for ii in range(max_tree_depth):
