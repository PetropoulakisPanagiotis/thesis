...
class MapViewer(object):
    ...
    def update(self, refresh=False):
        ...
        self.q_unc.put(self.system.current.scale_aware_frame.canonical_uncertainty)
    ...
    def view(self):
        ...
        while not pangolin.ShouldQuit():
            ...
            if not self.q_image.empty():
                ...
                image_unc = self.q_unc.get()
                image_unc = colormap(image_unc)
                image_unc *= 255
                image_unc = cv2.flip(image_unc, 0)

                image = cv2.resize(image, (width, height))
                image_unc = cv2.resize(image_unc, (width, height))

                combined = np.zeros((height, width * 2, 3), dtype=np.uint8)
                combined[:, :width] = image
                combined[:, width:width + width] = image_unc
...
def colormap(inputs, name='seismic'):
