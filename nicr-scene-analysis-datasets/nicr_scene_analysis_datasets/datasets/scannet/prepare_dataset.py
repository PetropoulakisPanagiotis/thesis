    ...
    def export_color_images(self, output_path: str, image_size=None, frame_skip=1, blacklist=None) -> None:
        ...
        for i in range(0, len(self.frames), frame_skip):
            ...
            with open(path, 'wb') as f:  # save bytes to file
                f.write(byts)
            """
            img = cv2.imdecode(np.frombuffer(byts, np.uint8), cv2.IMREAD_UNCHANGED)
...
class NICRImageZip:
            ...
            with self.zip.open(inf) as f:  # load image from zip
                ...
                for lmap_key, subtasks in grouping_dict.items():  # iterate the items in the group
                    ...
                    for odir, cmap in subtasks:  # complete the rest of the task on the img
                        ...
                        if 'instance' in path:
                            self.remove_small_instances(pix)
    ...
    def __export_image(
        ...
        if pix.dtype == 'uint8' and color_map is not None:  # save grayscale with palette
            ...
            new_img = new_img.resize((640, 480), resample=Image.NEAREST)
            ...
            pix = cv2.resize(pix, (640, 480), interpolation=cv2.INTER_NEAREST)
...
def parse_scene(
    ...
    sens.export_color_images(
        os.path.join(out_paths['rgb_dir'], scene_name),
        frame_skip=base_subsample)
    sens.export_depth_images(
        os.path.join(out_paths['depth_dir'],  scene_name),
        frame_skip=base_subsample)
    sens.export_intrinsics_color(
        os.path.join(out_paths['intr_rgb_dir'], scene_name + '.json'))
    sens.export_intrinsics_depth(
        os.path.join(out_paths['intr_depth_dir'], scene_name + '.json'))
    sens.export_extrinsics(
        os.path.join(out_paths['extr_dir'],  scene_name),
        frame_skip=base_subsample)
    ...
    for mode in ScanNetMeta.INSTANCE_SEMANTIC_MODES:
        ...
        for n_class in ScanNetMeta.SEMANTIC_N_CLASSES:
            ...
            sem_zip.add_export(
                os.path.join(out_paths[f'sem_{mode}_{n_class}_dir'], scene_name),
            ...
            for n_class_color in SEMANTIC_COLORS_PER_CLASS[n_class]:
                sem_zip.add_export(
                    os.path.join(out_paths[f'sem_{mode}_{n_class_color}_dir'], scene_name),
                    conversion_dict[n_class],
    ...
    for mode in ScanNetMeta.INSTANCE_SEMANTIC_MODES:
        ...
        inst_zip.add_export(os.path.join(out_paths[f'inst_{mode}_dir'], scene_name))  # export instance img as is
    ...
    export_scene_class(
        ...
        os.path.join(out_paths['scene_class_dir']),
        scene_name,
    )
